...
import type { EtchASketchAction, EtchASketchCanvasState } from "../types/contracts";

type EtchASketchWorkspaceProps = {
  canvasState: EtchASketchCanvasState;
  onAction?: (action: EtchASketchAction) => void;
  readOnly: boolean;
  alphaX: number;
  alphaY: number;
  onClear?: () => void;
...
export function EtchASketchWorkspace({ canvasState, onAction, readOnly, alphaX, alphaY, onClear, onUndo, canUndo }: EtchASketchWorkspaceProps) {
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      // Independent axis filtering with different alpha values
      const fx = prev.x + alphaX * (target.x - prev.x);
      const fy = prev.y + alphaY * (target.y - prev.y);
  ...
  }, [readOnly, alphaX, alphaY, onAction]);
  ...
  function handlePointerUp() {
    ...
    onAction?.({ action: "stroke_end", strokeId: strokeIdRef.current, timestampMs: Date.now() });
  }
